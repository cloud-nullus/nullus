...
- **스택별 파이프라인 조회 라우트가 등록되지 않았다** (`cmd/api/main.go`): `PipelineHandler.RegisterStackRoutes` 가 정의만 되고 호출되지 않아 `GET /api/v1/stacks/:stackId/pipelines` 가 404 였다. 프론트가 `?stack_id=` 쿼리 방식을 쓰고 있어 드러나지 않았다.
