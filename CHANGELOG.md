...
- **에어갭 설치의 Trivy 서버가 취약점 DB 를 인터넷에서 받으려 해 스캔이 전부 실패하던 것** (`internal/stack/adapter/helm`, `airgap/`): 에어갭 스택은 API 가 설치하는데(`29-install-stacks-via-api.sh`), 내부 미러 경로는 그 경로가 읽지 않는 `stack-values/trivy.yaml` 에만 있어 서버의 `TRIVY_DB_REPOSITORY` 가 `ghcr.io/aquasecurity/trivy-db` 로 남았다. 파일의 값도 호스트 주소 `localhost:5001` 이라 파드 안에서는 닿지 않았고, 계약 테스트는 그 값이 반입 경로와 같은지만 봤다. 에어갭 모드(`NULLUS_HELM_OCI_REGISTRY`)면 API 가 같은 레지스트리의 루트(`kind-registry:5000/aquasecurity/trivy-db`)와 `TRIVY_INSECURE=true` 를 넣고, YAML 오버라이드가 있는 분기에서도 빠지지 않는다. `stack-values/trivy.yaml` · 반입 스크립트 안내 · 설계 문서 §7.2 를 맞추고, 계약 테스트가 클러스터 안 주소와 코드 값을 함께 본다. 같은 이유로 **CI 잡의 trivy client 가 Java DB 를 인터넷(mirror.gcr.io)에서 받으려 해**, Maven 메타데이터가 없는 JAR 이 든 이미지의 스캔이 에어갭에서 FATAL 로 끝났다 — 에어갭 모드면 GitLab 스캔 잡 · Jenkins 스캐너 단계에 `TRIVY_JAVA_DB_REPOSITORY`(같은 내부 레지스트리)를 싣는다. 두 미러 경로 규칙은 `internal/shared/domain` 이 소유한다.
