...
- **에어갭 스택 설치가 시크릿 평면 첫 단계에서 멈추던 것 — 설치 차트가 카탈로그에 없음** (`airgap/scripts/pre/pull-charts-catalog.sh`, nullus-plan#82): 에어갭 설치는 차트를 내부 OCI 레지스트리에서만 받는데(`installAirgapOCI`), 그 레지스트리에 올리는 카탈로그에 설치 코드의 차트 7개가 없었다 — `openbao` 0.28.4 · `external-secrets` 2.7.0 · `postgresql` 16.7.27(모든 스택), `prometheus-operator-crds` 18.0.0 · `gitea` 12.7.0 · `jenkins` 5.9.54 · `nexus-repository-manager` 64.2.0. 설치 코드와 같은 저장소 · 버전으로 추가하고(모두 `helm pull` 확인), 헤더의 "openbao 는 차트 없음" 설명을 바로잡는다. `DefaultChartSpecForStep` 의 차트가 카탈로그에 없거나 버전 · 저장소가 다르면 `TestAirgapChartCatalog_CoversEveryInstallerChart` 가 막는다.
