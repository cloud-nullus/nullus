...
- [x] **F8-UIUX-StatusBadgeColors — 완료 (2026-04-21):** `stack-list-page.tsx` inline `STATUS_STYLES` 를 `utils/status-style.ts` 공유 util 로 추출. `rolled_back` 을 기존 grey → amber 로 교체해 `cancelled` 와 시각 구분. `RealStackView` (`stack-deployment-logs-page.tsx`) 도 동일 팔레트 사용 — inline hard-coded tone 삭제하고 `getStatusStyle(status)` 사용 + `data-testid="real-stack-status-badge"`. 4 단위 테스트 (11 키 존재 / failed red / rolled_back amber ≠ cancelled grey / 미지 key 는 pending fallback).
- [x] **F8-UIUX-MatrixListOps — 완료 (2026-04-21):** `stack-versions-page.tsx` 에 검색 입력 + 상태 필터(`NativeSelect`) 를 `items.length > 5` 일 때만 렌더. `filteredMatrices` memo 로 id/name 대소문자 무시 부분일치 + 상태 필터 적용. 제목 영역 우측에 color dot legend(`LegendDot` 내부 컴포넌트, `hidden md:flex`) 3종. i18n `stackVersionsAdmin.filter.*` + `stackVersionsAdmin.legend.*` (ko/en). 기존 5 + 신규 3 = 8 테스트 green.
- [x] **F8-UIUX-MatrixEditValidation — 완료 (2026-04-21):** `matrix-edit-modal.tsx` 에 인라인 validation (`validateFields`) + unsaved-changes close 가드. ID는 `/^[a-z0-9][a-z0-9-]{1,63}$/`, Name ≥2 글자, K8s 필드 `/^v\d+\.\d+(\.\d+)?$/`. 에러가 있으면 `Input.error` 에 메시지 + `canSubmit=false`. `initialSnapshot`/`currentSnapshot` JSON 비교로 `isDirty` 판정 → Cancel/ESC/backdrop 경로의 `handleCloseWithGuard` 가 `window.confirm` 으로 확인. i18n `stackVersionsAdmin.modal.validation.*` + `unsavedPrompt` (ko/en). 기존 3 + 신규 4 = 7 테스트 green.
- [x] **F8-UIUX-RealTimeline — 완료 (2026-04-21):** `RealStackView` 에 5 stage timeline(Queued/Provisioning/Deploying/Validating/Completed) 추가. `deriveRealStages(status)` 가 `stack.status` 단일값을 `{stages, terminal}` 로 파생 (pending/validating/installing+configuring/health_check+running/completed/failed/rolling_back/rolled_back/cancelled). Timeline 컨테이너에 `data-testid="real-timeline"` + `data-terminal` 속성. 기존 4 + 신규 3 = 7 테스트 green (pending=null terminal / failed=failed / completed=completed). legacy mock 분기는 그대로 보존.
- [x] **F8-UIUX-RetryFeedback — 완료 (2026-04-21):** `RetryStackButton` pending 스피너(`Loader2` animate-spin) + 3단계 toast 전이. Click → `toast.loading(pendingMsg)` → id 기억. onSuccess → `toast.success(msg, {id})`, onError `DEPLOY_COMPAT_FAIL` → `toast.error(msg + issues, {id, action: {label, onClick → navigate(/stack/install?stackId=...)}})`, onError warn-ack → `toast.dismiss(id)` 후 기존 modal, 기타 에러 → `toast.error(msg, {id})`. i18n `stackList.retry.toasts.{pending,fixAction}` (ko/en). 기존 4 + 신규 3 = 7 테스트 green.
- [x] **F8-UIUX-WarnAckPersist — 완료 (2026-04-21):** 신규 `utils/warn-ack-storage.ts` (`warnAckKey`, `readAck`, `writeAck`) — `nullus.warnAck.{client|server}.{stackName}.{clusterId}.{verdictHash}` 형식의 `sessionStorage` 키. verdictHash 는 issue 배열의 djb2. `stack-install-page.tsx` 양쪽 ack 체크박스(client/server) mount + verdict 변경 시 storage 에서 복원, onChange 시 write. sessionStorage 예외는 `try/catch` 로 삼켜 state-only fallback. 5 단위 테스트 (key 결정론 / 이슈 변경 시 key 변화 / client vs server namespace / round-trip / storage 예외 noop).
- [x] **F8-UIUX-A11y — 완료 (2026-04-21):** `components/ui/modal.tsx` 에 focus trap + prev-element focus restore. `cardRef` 로 포커스 영역 스코프. Tab/Shift+Tab 이 첫↔끝 순환, open 시 `requestAnimationFrame` 뒤 첫 focusable 자동 포커스. `stack-install-page.tsx` 양쪽 ack checkbox 에 `aria-label` 추가 (i18n `stackInstall.compatibility.gate.ackAria` / `stackInstall.compatibility.serverVerdict.ackAria`, ko/en). `stack-list-page.tsx` 카드 Stack Name 과 DataTable cell 에 `truncate` + `title` 툴팁. `modal.test.tsx` 3 신규 케이스 (autofocus / Tab wrap / restore).
- [x] **F8-UIUX-EmptyLoading — 완료 (2026-04-21):** 신규 `components/ui/skeleton.tsx` (`animate-pulse` primitive) + 1 단위 테스트. `stack-versions-page.tsx` 로딩 상태에 `Skeleton` 5줄 placeholder, 비어 있는 상태(`sortedMatrices.length === 0`)에 `FolderOpen` 일러스트 + 안내 문구 empty-state(`data-testid="matrix-empty-state"`). `stack-install-page.tsx` Auto Select 섹션 상단에 클러스터 미선택 시 amber hint 배너(`data-testid="auto-select-no-cluster-hint"`). i18n `stackVersionsAdmin.empty.{title,hint}` (ko/en). 기존 8 + 신규 1(empty) + 1(skeleton util) = 10 테스트 green.
- [x] **F8-UIUX-Polish — 완료 (2026-04-21):** `Button` 루트에 `whitespace-nowrap`, `Modal` 타이틀 `<h2>` 에 `break-keep` 적용 (i18n 길이 차로 인한 깨짐 #21 완화). `stack-install-page.tsx` 의 server verdict pass 분기는 full panel 대신 `"✓ passShort"` 1줄 배너로 축소(`data-state="pass"`). `matrix-edit-modal.tsx` 에 duplicate category 가드 (`duplicateCats` Set + `hasDuplicateCats` → `canSubmit=false` + `data-testid="matrix-dup-warn"`). i18n `stackInstall.compatibility.serverVerdict.passShort` + `stackVersionsAdmin.modal.validation.duplicateCategory` (ko/en). 기존 + 신규 1(dup) = 82 테스트 green.
- [x] **F8-UIUX-RetryAuditSurface-Frontend — 완료 (2026-04-21):** 신규 `RetryHistoryEntry` 타입 + `useStackRetryHistory(stackId)` 훅 (`queryKey: ['stack-retry-history', stackId]`, `staleTime: 30_000`). `stack-deployment-logs-page.tsx` 내부에 `RetryHistoryPanel` inline 컴포넌트 (items.length===0 시 렌더 스킵, 초기 3행 + "더 보기" 토글, `data-testid="retry-history-panel"` / `retry-history-toggle`). i18n `stackDeployment.retryHistory.{title,timestamp,verdict,issues,expand,collapse}` ko/en. 3 신규 단위 테스트 (panel hidden on empty / renders rows with verdict+issues / expand toggle 3→5). 전체 10 테스트 green.
- [x] **F8-UIUX-RetryAuditSurface-Backend — 완료 (2026-04-21):** 신규 `audit.Reader` interface + `audit.TimedEntry` (AuditEntry + ID + Timestamp). `MemorySink.ListByResource` (recordingMeta sidecar + atomic seq id + `WithClock` override, timestamp DESC 정렬 + deep copy). `AuditLogger.ListByResource` (audit_logs 테이블 SQL, DB id/created_at 반환). 신규 `RetryHistoryHandler` (`GET /api/v1/stacks/:id/retry-history`) — action=retry 만 필터 후 retryHistoryItem (previousState/acknowledgeWarnings/verdict/issueCodes) 로 매핑. issue_codes `[]string`/`[]any` 모두 허용. cmd/api/main.go 에 `NewRetryHistoryHandler(auditLogger)` + `RegisterRoutes(stacks)` 등록. 4 MemorySink 테스트 + 3 handler 테스트 green.
- [x] **F8-UIUX-KeyboardHints — 완료 (2026-04-21):** 신규 `hooks/use-keyboard-shortcut.ts` (`window.keydown` 리스너, `enabled` / `allowInInput` 옵션, 기본 input/textarea/contenteditable focus 시 skip). 신규 `components/shortcut-help-modal.tsx` (`SHORTCUT_REGISTRY` 단일 소스 + 2열 테이블 렌더). `app/layout.tsx` 에 `?` 글로벌 바인딩 + 우하단 `shortcut-badge` 고정 버튼 + ShortcutHelpModal 렌더. `stack-list-page.tsx` 에 `n` → `/stack/install` 네비게이션. i18n `shortcuts.{badge,help.*,newStack,scope}` ko/en. 4 훅 테스트 + 1 modal 테스트 green.
- [x] **F8-UIUX-E2EDeepScenario — 완료 (2026-04-21):** `web/e2e/stack-retry-scenario.spec.ts` 신규 1 케이스 (`@stack-critical`, Tier B backend-free): deploy-fail stack → Retry click → `DEPLOY_COMPAT_WARN_UNACK` (issue: TOOL_ARCH_UNSUPPORTED) → warn modal 노출 → ack check → Retry 재제출 → `acknowledge_warnings: true` body + 200 → 성공 토스트(`재배포를 시작|Redeploy started`) 노출. 2-request 순차 스텁 + retryBodies assertion. Tier B 기존 3 + 신규 1 = 4/4 green. Tier A 선재 2 실패는 live Postgres 필요 — 본 Phase 에서 수정하지 않음.
