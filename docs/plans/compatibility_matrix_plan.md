...
- [ ] **F8-Phase5-DOMTest:** `web/src/features/admin/pages/stack-versions-page.test.tsx` 신규 — New → modal open, Edit → pre-filled values, Delete → Confirm dialog → useDeleteMatrix.mutate 호출 DOM-레벨 스모크. Phase 5 재개 범위에서 스코프 축약으로 빠졌음. 현재 handler 6 케이스 + useMutation 훅 tsc 통과로 API contract 는 커버됨.
- [ ] **F8-DeployError-Dedup:** `stack-install-page.tsx` 내 inline `extractDeployCompatError` 제거 — Phase 3-Retry-UI 에서 `utils/deploy-error.ts` 로 추출된 공유 버전으로 완전 이전. 현재는 두 곳에 동일 시맨틱 중복. 작은 cleanup PR 로 분리.
- [ ] **F8-RetryUI-E2E:** Playwright `@stack-critical` Retry 스모크 신규 — `failed` 상태 stack fixture seed + `/stack/list` 에서 Retry 버튼 가시성 + click → API 호출 검증 (MSW 스텁 200). Kind 의존 없는 순수 UI 스모크.
- [ ] **F8-Retry-Toast:** Retry 성공/실패 전역 toast — 현재 `RetryStackButton` 은 `onRetried` 콜백만 호출. 전역 toast 시스템(sonner 등) 도입 시 공통 알림으로 완성.
- [ ] **F8-DeploymentLogs-Retry:** `stack-deployment-logs-page.tsx` Retry 버튼 통합 — 현재 mock data 기반이므로 real status prop 리팩토링 후 `RetryStackButton` 재사용.
