...
**작성일**: 2026-08-22 · **갱신**: 2026-08-23 — 2트랙 병행 구조로 재편
**상태**: 중간점검 결정 반영 — "빠른 시일 내 추가 구현"으로 계획 진행
...
**범위**: 통합 `nullus` CLI(1급 명령 중심) + Nullus MCP 서버
...
| # | 태스크 | 산출물 / 완료 기준 | 크기 |
|---|--------|-------------------|------|
| 0-1 | **ADR-0001 갱신** — 중간점검 결정(조기 착수) 반영, 상태 Proposed→Accepted | ADR에 착수 시점 변경 사유 기록. F5·F6 API 변경 시 CLI/MCP 추적 비용을 감수한다는 결정 명시 | S |
| 0-2 | **`stack.yaml` 선언형 스키마 설계** — 서버에도 없는 개념, CLI 간판 명령의 성립 조건 | 스키마 문서(`docs/20_개발가이드/`): 필드 정의, `apiVersion` 버저닝, 마법사 5단계 입력→배포 API payload 대응표(트랙 B의 `stack_deploy` tool도 이 대응표 재사용). 웹 마법사 "yaml 내보내기" 기능 여부 결정 포함 | M |
| 0-3 | **automation 계약 확정** — exit code 규약(배포 실패 vs CLI 오류 구분), `-o json` 안정성 보장 범위, `--wait` 시맨틱 | 컨셉 문서 §8 열린 질문을 결정으로 전환해 문서화 | S |
| 0-4 | **MCP 연동 설계 문서 작성** (현재 부재) | `docs/20_개발가이드/Nullus_MCP_설계.md`: Go SDK 선정(공식 `modelcontextprotocol/go-sdk` 우선 검토), tool 표면 v1 확정, 읽기/변경 tool 구분과 변경 tool 허용 정책, transport(stdio 우선·HTTP는 범위 밖), 인증 경로(토큰 캐시·bootstrap 토큰 재사용), **패키징 결정(단일 바이너리 서브커맨드 vs 별도 바이너리)** | M |
| 0-5 | **컨벤션 정비** — 브랜치/PR 모듈 목록에 `cli` 추가 여부 결정 (현재 `stack\|cicd\|admin\|auth\|o11y\|product\|ui\|shared\|infra`뿐) | `Nullus_PR_커밋_컨벤션.md` + `.github/workflows/lint-review.yml` 정합. 미추가 시 `shared` 사용 결정 명시 | S |
...
| # | 태스크 | 완료 기준 | 크기 | 의존 |
|---|--------|----------|------|------|
| B-1 | MCP 서버 골격 (stdio) — 0-4 선정 SDK 통합, 공유 기반(S-1·S-2·S-3)만 사용 | MCP Inspector로 initialize/list_tools 확인 | M | 0-4, S-1, S-2 |
...
- 2급 명령(`template`/`config`/`compat`/`token-source`/`alert` CLI) — MCP 읽기 tool이 일부 수요를 대신 흡수
...
- **트랙 A 크리티컬 패스**: 0-2(스키마) → A-5(stack deploy) → A-6 → A-7
