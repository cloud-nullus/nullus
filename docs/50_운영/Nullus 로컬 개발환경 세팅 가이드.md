...
- **신속성**: Vite HMR과 빠른 서버 재기동으로 코드 변경 사항을 즉시 확인하고 피드백 루프를 단축합니다.
...
│  │                                                                │ │
│  │  - Context: kind-nullus-platform / kind-nullus-develop         │ │
│  │  - API Server: :6443                                           │ │
...
| Go API | 8090 | HTTP | 백엔드 REST API 서버. 프론트엔드와 통신. |
...
| WebSocket | 8090 | WS | 설치 로그 실시간 스트리밍용 (API 서버 내장). |
...
6. **저장소 클론**:
   ```bash
   git clone git@github.com:cloud-nullus/draft.git
   cd draft
   ```
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `NULLUS_DB_HOST` | `localhost` | 로컬 Docker로 실행 중인 DB 주소. |
| `NULLUS_DB_PORT` | `5433` | PostgreSQL 호스트 포트. |
| `NULLUS_DB_NAME` | `nullus` | 애플리케이션용 데이터베이스 이름. |
| `NULLUS_DB_USER` | `nullus` | DB 관리자 계정. |
| `NULLUS_DB_PASSWORD` | `nullus_dev` | 개발용 비밀번호 (보안 주의). |
| `API_PORT` | `8090` | 백엔드 서버가 리스닝할 포트. |
| `VITE_API_BASE_URL` | `http://localhost:8090/api/v1` | 프론트엔드 API 기본 URL. |
...
> 수동 실행이 필요하면 `make dev-up`, `make run`, `make web-dev`를 개별로 사용할 수 있습니다.
...
| 명령어 | 상세 설명 |
|--------|-----------|
| `make dev` | Docker 인프라 기동 + 마이그레이션 적용 (`dev-up` + `migrate-up`). |
| `make dev-up` | Docker 인프라만 기동 (PostgreSQL/Redis/MinIO/Keycloak). |
| `make dev-down` | Docker 인프라 종료. |
| `make dev-clean` | Docker 인프라 + 볼륨까지 삭제. |
| `make run` | API 서버 빌드 후 실행 (`.env.dev` 자동 로드). |
| `make run-dev` | API 서버 실행(환경 파일 자동 로드). |
| `make web-dev` | 프론트엔드 dev 서버 실행. |
| `make build` | 전체 프로젝트를 빌드하여 `dist/` 폴더에 결과물을 생성합니다. |
| `make test` | 모든 유닛 테스트를 실행합니다. |
| `make test-integration` | Go integration(E2E 태그) 테스트 실행. |
| `make web-test` | `web` 폴더에서 Vitest 실행. |
...
1. **서버 실행**: `make run` 또는 `./scripts/runbook_local.sh up`로 API를 실행합니다.
2. **코드 수정**: `internal/` 하위 코드를 수정하면 서버가 1~2초 내에 재시작됩니다.
3. **구조적 설계**:
   - `internal/*/adapter/handler/`: Echo 핸들러 정의. 요청 바인딩 및 응답 처리.
   - `internal/*/usecase/`: 비즈니스 로직 핵심. 인터페이스 기반 설계.
   - `internal/*/adapter/repository/`: DB 접근 구현.
4. **테스트 작성**:
...
1. **파일 생성**: `db/migrations/`에 새 버전의 `*.up.sql`, `*.down.sql` 파일을 생성합니다.
2. **SQL 작성**:
...
#### 브랜치 네이밍 규칙
- `feat/<module>/<description>`: 기능 개발 (예: `feat/stack/install-workflow`)
- `fix/<module>/<description>`: 버그 수정 (예: `fix/cicd/rollback-pvc`)
- `docs/<module>/<description>`: 문서 작업

#### 커밋 메시지 템플릿
```text
feat(stack): 사용자 로그인 API 구현

- Echo 핸들러 및 JWT 발급 로직 추가
...
Nullus API 스펙은 `api/openapi.yaml`을 기준으로 관리합니다.
...
#### 문서 갱신 및 확인
1. 코드/API 변경 후 `api/openapi.yaml`을 함께 갱신합니다.
2. 서버 실행 (`make run` 또는 `./scripts/runbook_local.sh up`).
3. 주요 API endpoint를 curl 또는 smoke 테스트로 검증합니다.
...
- **실행**: `./scripts/runbook_local.sh kind-up` + `cd web && npx playwright test --reporter=list`
...
**Q: `make run` 또는 `runbook_local.sh up` 실행 시 포트 충돌 에러가 납니다.**
- **증상**: `listen tcp :8090: bind: address already in use`
- **해결**:
  - 해당 포트를 점유 중인 프로세스를 찾아 종료합니다.
  - macOS: `sudo lsof -i :8090` -> `kill -9 <PID>`
  - Windows: `netstat -ano | findstr :8090` -> `taskkill /F /PID <PID>`

**Q: WSL2에서 로컬 호스트 접속이 안 됩니다.**
- **원인**: WSL2는 별도의 가상 IP를 가집니다.
- **해결**: `.env.dev`에서 `NULLUS_DB_HOST`를 확인하거나, Docker Desktop의 WSL Integration이 켜져 있는지 확인하세요.
...
- **해결**:
  ...
  - `./scripts/runbook_local.sh kind-down` 후 `./scripts/runbook_local.sh kind-up`으로 재시도.

**Q: `kubectl` 명령어가 엉뚱한 클러스터로 전송됩니다.**
- **해결**: 컨텍스트를 명시적으로 전환하세요.
  ```bash
  kubectl config use-context kind-nullus-platform
  ```
...
- **원인**: `.env.dev`의 `ENCRYPTION_KEY`가 변경되었거나 설정되지 않음.
- **해결**:
  - `.env.dev` 파일에 32바이트 길이의 `ENCRYPTION_KEY`가 있는지 확인하세요.
  - 키를 변경하면 기존에 DB에 저장된 kubeconfig는 모두 다시 등록해야 합니다.

### 7.7 Helm 차트 다운로드 실패
- **증상**: kind 기반 로컬 배포/테스트 중 Helm 차트 fetch 실패.
...
- **해결**:
  ...
  - `.env.dev`의 `VITE_API_BASE_URL`이 올바른지 확인.
  - 브라우저 캐시 삭제 후 재시도.
...
9. [ ] **테스트 실행**: `go test ./... -count=1` 및 `cd web && npx vitest run`이 모두 통과하는지 확인합니다.
...
### 9.2 문서화 의무
- **API**: 새로운 API를 만들거나 기존 API를 수정하면 `api/openapi.yaml` 스펙을 함께 업데이트합니다.
...
- **환경 변수**: 로컬 실행은 `.env.dev`를 기준으로 하며, 새로운 환경 변수가 추가되면 `.env.example`에도 반영합니다.
