...
| 서비스 | 포트 | 프로토콜 | 설명 |
|--------|------|----------|------|
| React (Vite) | 3000 | HTTP | 프론트엔드 HMR 개발 서버. 브라우저 접속 주소. |
| Go API | 8080 | HTTP | 백엔드 REST API 서버. 프론트엔드와 통신. |
| PostgreSQL | 5432 | TCP | 메타데이터 저장용 DB. |
...
```bash
# Kind 클러스터 생성
# kind-config.yaml은 프로젝트 루트에 포함되어 있습니다.
...
6. [ ] **인프라 실행**: `docker compose up -d postgres` 명령으로 DB를 실행합니다.
7. [ ] **DB 초기화**: `make migrate-up` 명령으로 테이블을 생성합니다.
8. [ ] **개발 서버 실행**: `make dev` 실행 후 브라우저에서 `localhost:3000` 접속이 되는지 확인합니다.
