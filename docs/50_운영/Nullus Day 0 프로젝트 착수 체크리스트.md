...
# - Test Users: admin@nullus.dev, operator@nullus.dev, viewer@nullus.dev
# - User Groups: admin, operator, viewer (각 역할 매핑)
```
...
- [ ] groups/roles scope 검증 계획 수립
  ...
  - [ ] 사용자 역할(admin/operator/viewer)과 K8s RBAC 매핑 전략
...
CREATE TABLE clusters (
    ...
    org_id           UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    name             VARCHAR(100) NOT NULL,
    type             VARCHAR(20) NOT NULL CHECK (type IN ('pipeline', 'target')),
    kubeconfig_enc   BYTEA NOT NULL,
    endpoint         VARCHAR(500),
    namespace        VARCHAR(63),
    auth_method      VARCHAR(50) DEFAULT 'kubeconfig',
    k8s_version      VARCHAR(20),
    status           VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('connected', 'pending', 'unreachable', 'auth_failed')),
...
CREATE TABLE stack_configs (
    ...
    name            VARCHAR(100) NOT NULL,
    golden_path_id  VARCHAR(100),
    config_json     JSONB NOT NULL DEFAULT '{}',
    status          VARCHAR(20) DEFAULT 'draft' CHECK (status IN ('draft', 'deploying', 'deployed', 'failed', 'rolling_back')),
    current_version INTEGER DEFAULT 0,
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    updated_at      TIMESTAMPTZ DEFAULT NOW()
);

-- Stack Config Versions (이력 관리)
CREATE TABLE stack_config_versions (
    ...
    created_at       TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(stack_config_id, version_number)
);

-- Deployments
CREATE TABLE deployments (
    id               UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    stack_config_id  UUID NOT NULL REFERENCES stack_configs(id),
    type             VARCHAR(20) NOT NULL CHECK (type IN ('stack', 'pipeline')),
...
CREATE TABLE pipeline_deployments (
    id                 UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    pipeline_config_id UUID NOT NULL REFERENCES pipeline_configs(id),
    version            INTEGER NOT NULL,
    status             VARCHAR(20) DEFAULT 'pending',
    k8s_objects        JSONB DEFAULT '[]',
...
`db/migrations/001_init.down.sql`:

```sql
DROP TABLE IF EXISTS alert_configs;
DROP TABLE IF EXISTS pipeline_deployments;
DROP TABLE IF EXISTS pipeline_configs;
DROP TABLE IF EXISTS deployment_logs;
DROP TABLE IF EXISTS deployments;
DROP TABLE IF EXISTS stack_config_versions;
DROP TABLE IF EXISTS stack_configs;
