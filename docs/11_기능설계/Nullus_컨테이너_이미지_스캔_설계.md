...
**어떻게 들어오나** — 잡이 판정을 요청하면서 리포트를 함께 보낸다(§5.4 의 `nullus-ci scan-gate`). 적재 경로가 하나고 CI 종류를 모른다. 초안은 GitLab · Jenkins · GitHub Actions 세 API 로 아티팩트를 각각 회수해야 했다.
...
| Trivy Operator (실행 중 워크로드 스캔) | 게이트가 아니라 탐지. **§7.2 의 DB 미러를 그대로 쓴다** | 경로 2 이후 |
