...
작성일: 2026-09-01 (2026-09-02 개정 — **범위 확대: 설치된 OSS 내부 데이터 포함**)
기준 실측: 마이그레이션 최신 버전 74 · 에어갭 번들 이미지 74개
...
축소 리허설이 검증하지 못한 것을 명시한다. **이것들이 끝나야 B4-1 완료다.**

| 항목 | 왜 축소 리허설로 안 되나 |
|---|---|
| **실제 OSS 데이터** — GitLab 커밋 · Harbor 이미지 digest · Jenkins 빌드 이력 · MinIO 오브젝트 | 도구별 정합성(Gitaly, 레지스트리 blob+DB 이중 정합)은 그 도구가 있어야 드러난다 |
| **OpenBao 금고** | 실제 금고가 필요하다. 축소 리허설은 KV 를 no-op 으로 뒀다 |
| **RTO 실측** | 데이터량이 지배한다. 256KiB 로 잰 시간은 근거가 아니다 |
| **정지 창 실측** | 위와 같다. 100Gi 급에서 몇 분인지가 정책의 수용 가능성을 정한다 |
| **다중 노드** | §3.5 의 "RWO PVC 가 같은 노드에" 전제가 깨지는 경우 |
| **실제 볼륨 규모의 egress** | §10.2.1b 는 73KB 를 보냈다. 100Gi 를 보내는 링크 대역폭은 별개 문제다 |
...
| ~~**Q9**~~ | ~~egress 경로가 실제로 열리는가~~ | ✅ **해소** — 인클러스터 리허설로 확인(§10.2.1b). 단 **에어갭 실환경**의 방화벽은 B4-2 에서 별도 확인 | — |
| **Q10** | 아카이버 이미지 — 헬퍼 파드는 `busybox:1.37`(번들 보유)로 확정. `mc` 태그 불일치는 별도 이슈 | ✅ 해소 | ❌ |
| **Q11** | 무중단·직접 업로드가 필요해질 때 S3 SSE-C 로 Job 에 키를 넘길지 (§3.6 정정) | Velero 재도입(Q5)과 같은 시점 | ❌ |
...
| **Q6** | **담당자** | **착수 전** | ⚠️ **유일한 실질 차단 요인** |

> **기술적 차단 요인은 남아 있지 않다.** Q1·Q9·Q10 은 각각 우회·검증·구현 내 결정으로 흡수됐다. 남은 것은 **Q6 — 담당자 미지정**이며, 10월 시한 항목이므로 이것부터 정해야 한다.
...
| **B0-6** 설계 문서 | ✅ | 본 문서 |
| B1-1 ~ B1-4 | 설계 완료 | §6·§7·§8 |
| **B2-1** OpenBao 백업/복구 | **전제 수정됨** | §3.2 — snapshot API 부재 → KV 논리 export + 정지 시 PVC 동봉 |
| B2-2 unseal 키 취급 | 설계 완료 | §5.2·§5.3 |
| B3-1 ~ B3-4 | 설계 완료 | §4.5·§7.2·§8(scheduler)·§9(F10) |
| **B4-1** 복구 리허설 | ⚠️ **축소 규모 통과**(§10.2.1, 결함 5건 발견·수정) · **실환경 리허설 남음**(§10.2.2) | §10 |
| B4-2 에어갭 검증 | 절차 확정 | §10.3 |
| **신규 제안** | — | **B1-5 볼륨 정지 백업/복원** · **B1-6 네임스페이스 리소스 덤프/복원** (E1·D1 편입) |
| **분리 제안** | — | **Keycloak DB 통합**(§1.2 결정 1) · **`mc` 이미지 태그 정합**(§3.6 부수 발견) · **로그 테이블 보존 정책**(Q4) — 셋 다 이 EPIC 밖 |

### 12.1 착수 순서 권고 (10월 시한 기준)

| 순서 | 작업 | 이유 |
|---|---|---|
| **0** | **키 자재 4종 에스크로 런북**(`ENCRYPTION_KEY` · unseal key · 백업 암호화 키 · **목적지 자격증명**) + **외부 오브젝트 스토리지 확보 및 egress 확인**(Q9) | 런북은 코드 없이 오늘 가능하고 그 자체로 현재 리스크를 낮춘다. egress 는 막혀 있으면 방화벽 변경 리드타임이 있으므로 **먼저 확인만이라도 해 둔다** |
| 0.5 | **(별도 이슈) Keycloak DB 통합** — §1.2 결정 1 | **선택이지만 앞에 둘수록 이득.** 1번 착수 전에 끝나면 버전 함정·연결 지점이 줄어든다. 늦어져도 백업 구현은 막지 않는다 |
| 1 | B1-1~B1-4 — 플랫폼/Keycloak DB + B2-1 KV export | 작고 검증이 쉽다. 여기서 매니페스트·암호화·이력 골격이 선다 |
| 2 | **B1-5 / B1-6 — 정지 백업과 볼륨·리소스 복원** | E1 의 본체. **가장 큰 작업** |
| 3 | **B4-1 리허설 (1차)** | **UI·스케줄보다 먼저.** 여기서 실패하면 설계를 고쳐야 하고, 그 전에 만든 UI 는 버려진다 |
| 4 | B3-1 / B3-3 — 스케줄(야간 정지 창) + 실패 알림 | RPO 24h 를 실제로 보장하고 F10 을 막는다 |
| 5 | B3-2 / B3-4 — UI + 런북 | 정지 창 고지를 UI 에 반드시 넣는다 |
| 6 | **B4-1 (2차) + B4-2** | 완료 판정 |

> 3번을 앞으로 당긴 것이 이 순서의 핵심이다. 이 EPIC 의 존재 이유가 *"복원이 안 됐던 경험"* 인 이상, **검증을 마지막에 두면 같은 실패를 반복한다.**
