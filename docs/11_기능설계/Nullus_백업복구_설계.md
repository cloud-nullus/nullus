...
| Go S3 SDK 를 API 파드에 | ❌ 탈락. 데이터가 API 파드를 경유하면 메모리·네트워크가 이중으로 든다. **볼륨을 마운트한 Job 이 직접 올리는 것이 맞다** |
...
| **Q10** | 아카이버 이미지 구성 — `mc` 에 `tar`/`zstd` 가 있는가(§3.6) | **B1-5 첫 결정 항목** | ❌ |
