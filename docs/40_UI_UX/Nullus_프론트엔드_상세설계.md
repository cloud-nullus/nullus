...
> **이 문서는 2026-03-14 설계안이다.** 상태 관리·라우팅·API 레이어·i18n 의 **구조**는
> 지금도 유효하지만, 아래 세 가지는 실제 구현과 다르므로 그대로 따라 하지 않는다.
>
> | 이 문서 | 실제 |
> |---|---|
> | 1장의 `npx shadcn@latest init` / `add ...` | **shadcn/ui 는 도입된 적이 없다.** `@radix-ui/*` 도 `shadcn` CLI 도 들어온 적이 없고, `components/ui/*` 는 손수 구현이며 2026-08-11 개편에서 **내부만** MUI 9 + Emotion 으로 바뀌었다 |
> | 2장 디렉토리 구조의 "shadcn/ui 기반 (자동 생성)" 주석 | 자동 생성 코드가 아니다. 직접 유지보수하는 프리미티브 15종이다 |
> | 색상값을 컴포넌트에 직접 적는 예제 | 색 리터럴은 **ESLint 가 막는다.** 단일 출처는 `web/DESIGN.md` 이고 거기서 파생된 CSS 변수(`var(--color-*)`)를 쓴다 |
>
> 현행 스택은 `Nullus_UI_UX_구현계획.md` 2.1,
> 화면별 정보는 `화면_정보_인벤토리.md`(자동 생성),
> 개편 경위는 `Nullus_UIUX_전면개편_기획안.md` 를 본다.
