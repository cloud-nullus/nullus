...
**최종 업데이트**: 2026-08-31
**현재 버전**: v0.4.1 (2026-08-10 릴리스). 그 뒤 약 3주치 작업이 `[Unreleased]` 에 쌓여 있다

> **진행 현황 (2026-08-31)** — 아래 타임라인은 3월 계획이다. 실제 릴리스는
> v0.2.0-alpha(3/28) → v0.3.0-alpha(7/28) → v0.4.0-alpha(8/9) → v0.4.1(8/10) 로 이어졌고,
> Phase 1 의 기능 축(F0~F10)은 모두 구현됐다. 일부는 계획을 넘어섰다 —
> Golden Path 템플릿은 목표 3개 대비 **9개**, 호환성 매트릭스는 **8개**다.
>
> 계획에 없던 축이 계속 늘고 있다.
>
> | 축 | 시점 | 내용 |
> |----|------|------|
> | 외부 SCM(GitHub) | ~2026-08-11 | 저장소 생성·워크플로 스캐폴딩·GHCR push |
> | 스택별 OpenBao 시크릿 평면 | ~2026-08-11 | OpenBao + External Secrets Operator 를 스택마다 배포 |
> | 토큰 회전 관리 | ~2026-08-11 | 회전·승인·일시중지 API 와 관리 화면 |
> | **Gitea + Jenkins 스택** | 2026-08-14 | SCM/CI 가 GitLab 전용이 아니게 됐다. 템플릿 2종(표준/Lite) |
> | **OpenTelemetry 관측 계층** | 2026-08-13 | 스택이 OTel Collector·Agent 를 세우고 배포 앱에 수집기 주소를 넣는다 |
> | **UI 전면 개편** | 2026-08-11~12 | DESIGN.md 단일 출처 → 토큰 파이프라인, 프리미티브 내부를 MUI 로, 하드코딩 색 1,426건 제거 |
> | **스택 OSS 의 SSO** | 2026-08-19 | Argo CD·Harbor·GitLab·Gitea·Jenkins·Grafana·MinIO 를 Keycloak 인증으로. 도구마다 비밀번호 우회로 유지 |
> | **와일드카드 TLS** | 2026-08-19 | cert-manager DNS-01 발급 + 기본 인증서 지정 |
> | **설치·삭제 신뢰성** | 2026-08-20~21 | 스택 전용 네임스페이스, PVC·네임스페이스 회수, 설치 로그 DB 영속화, 파이프라인 삭제의 이미지·앱·CI job 회수 |
> | **통합 `nullus` CLI + MCP** | 2026-08-22~ | `pkg/nullusclient` 공유 기반. Phase 0(설계 5건) 완료, Phase 1 진행 중 |
>
> 기능별 달성도는 `docs/10_제품기획/Nullus_기능목록.md` 0장,
> 남은 항목은 `docs/20_아키텍처/Nullus_설계_대비_미구현_항목.md` 를 본다.
> 릴리스별 변경은 `CHANGELOG.md` 가 단일 출처다.
>
> **주의** — F7(모니터링/알림)은 "구현" 으로 세지 않는다. 규칙 CRUD 는 되지만 알림이
> 실제로 나가지 않는다(기능목록 0.2). CLI·MCP 는 `docs/plans/2026-08-22-cli-mcp-구현-백로그.md`
> 가 진행 상황의 단일 출처다.
...
- [ ] CI/CD Webhook 트리거: GitHub/GitLab push·PR merge 이벤트로 파이프라인 자동 실행 (Pipeline에 trigger_type, webhook_secret 추가, `POST /api/v1/webhooks/github` 엔드포인트, HMAC 서명 검증)
...
| OpenBao Secret Control Plane | OpenBao 선배포 + OSS 토큰/시크릿 중앙관리 + 회전 자동화 | P0 |
