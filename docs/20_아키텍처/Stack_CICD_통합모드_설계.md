...
### 3.2 Runtime 배포 흐름 (Developer push 기준)
...
6. CI Platform: test (phase 2)
   단위 테스트 / 통합 테스트 수행
   test report -> Package Registry에 publish

7. CI Platform: security scan - SAST / SCA (phase 2)
   소스 정적 분석 (SAST)
   의존성 취약점 분석 (SCA)
   SBOM 생성 -> Package Registry에 publish

8. CI Platform: image build & push
   kaniko로 container image 빌드 (Docker 데몬 불필요, rootless)
...
10. CD Tool (Argo CD)
    Environment Repository 변경 감지 (polling, 기본 3분)
