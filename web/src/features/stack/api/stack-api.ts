...
const stackApiCalls = {
  ...
  getTemplate: (id: string) =>
    api.get<import('../../../types').StackTemplate>(`/stacks/templates/${id}`).then((r) => r.data),

  getList: (filters?: { status?: string; search?: string; include_deleted?: boolean }) =>
    api.get<{ items: import('../../../types').Stack[]; total: number }>('/stacks', { params: filters }).then((r) => ({
      ...r.data,
  ...
  estimateResources: (input: import('../../../types').CreateStackRequest['resources']) =>
    api.post<import('../../../types').ResourceEstimate>('/stacks/estimate', input).then((r) => r.data),
  ...
  getVersionDiff: (stackId: string, from: number, to: number) =>
    api.get<import('../../../types').StackVersionDiff>(`/stacks/${stackId}/history/diff`, { params: { versionA: from, versionB: to } }).then((r) => r.data),
  ...
  createTemplate: (request: TemplateMutationRequest) =>
    api.post<import('../../../types').StackTemplate>('/stacks/templates', request).then((r) => r.data),

  updateTemplate: (request: TemplateMutationRequest) =>
    api.put<import('../../../types').StackTemplate>(`/stacks/templates/${request.id}`, request).then((r) => r.data),
...
export function useStackRetryHistory(stackId: string | undefined) {
  return useQuery<{ items: import('../../../types').RetryHistoryEntry[] }>({
    ...
    queryFn: async () => {
      const res = await api.get<{ items: import('../../../types').RetryHistoryEntry[] }>(`/stacks/${stackId}/retry-history`)
      return res.data
    },
