...
const CICD_TYPES: Array<{ id: CicdType; label: string; description: string }> =
  [
    ...
    { id: "helm", label: "Helm", description: "Helm chart release" },
    {
      id: "cronjobJob",
      label: "Cronjob/Job",
      description: "Scheduled and one-time workloads",
    },
  ];
...
const TEMPLATE_DESCRIPTION_KO_TO_EN: Record<string, string> = {
  "React/Next.js 웹 프론트엔드 앱 템플릿":
    "React/Next.js web frontend app template",
...
function resolveCicdType(template: CicdTemplate): CicdType {
  const searchable =
    `${template.id} ${template.name} ${template.stages.join(" ")}`.toLowerCase();
  if (searchable.includes("helm")) return "helm";
  if (
    searchable.includes("cronjob") ||
    searchable.includes("cron job") ||
...
function resolveCapabilities(stages: string[]): string[] {
  const hasCapability = (capability: (typeof CAPABILITY_OPTIONS)[number]) =>
    stages.some((stage) => {
...
export function CicdTemplatePage() {
  ...
  const templates = (Array.isArray(apiTemplates) ? apiTemplates : [])
    ...
    .map((template) =>
      template.id === "web-backend-v1"
        ? { ...template, name: "User Custom Pipeline" }
        : template,
    );
  ...
  const prioritizedFiltered = filtered.slice().sort((a, b) => {
    const aPriority = PRIORITY_TEMPLATE_IDS.includes(
      a.id as (typeof PRIORITY_TEMPLATE_IDS)[number],
    );
    const bPriority = PRIORITY_TEMPLATE_IDS.includes(
      b.id as (typeof PRIORITY_TEMPLATE_IDS)[number],
    );
  ...
  const templatesByType = CICD_TYPES.map((type) => ({
    ...type,
    templates: prioritizedFiltered.filter(
      (template) => resolveCicdType(template) === type.id,
    ),
  }));
  ...
  const submitTemplate = () => {
    if (!form.name.trim()) {
      setFormError(
        t("cicdTemplatePage.errors.nameRequired", "Name is required."),
      );
      return;
    }
    ...
    if (form.stages.length === 0) {
      setFormError(
        t(
          "cicdTemplatePage.errors.stageRequired",
    ...
    const templateId =
      editingTemplateId ??
      (form.id.trim() || form.name.toLowerCase().replace(/\s+/g, "-"));
    ...
    if (editingTemplateId) {
      updateTemplate.mutate(payload, {
        ...
        onError: () =>
          setFormError(
            t(
              "cicdTemplatePage.errors.updateFailed",
    ...
    createTemplate.mutate(payload, {
      ...
      onError: () =>
        setFormError(
          t(
            "cicdTemplatePage.errors.createFailed",
  ...
  return (
    <div>
              ...
              <section key={type.id} aria-label={type.label}>
                <div className="mb-3 flex items-baseline gap-3">
                  ...
                  <span className="text-sm text-[var(--color-text-secondary)]">
                    {t(
                      `cicdTemplatePage.types.${type.id}.description`,
                      type.description,
                    )}
                  </span>
                </div>
                {type.templates.length === 0 ? (
                  <div className="rounded-[var(--card-radius)] border border-dashed border-[var(--color-border-default)] px-4 py-5 text-sm text-[var(--color-text-muted)]">
                    {t(
                      "cicdTemplatePage.typeEmpty",
                      "No templates in this type.",
                    )}
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <span className="text-[11px] font-semibold text-[var(--color-text-muted)]">
                              {t(
                                "cicdTemplatePage.card.capabilities",
                                "Capabilities",
                              )}
                            </span>
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              ...
                              {capabilities.length === 0 && (
                                <span className="text-xs text-[var(--color-text-muted)]">
                                  {t(
                                    "cicdTemplatePage.card.noCapabilities",
                          ...
                          <div className="mt-auto flex flex-wrap items-center justify-between gap-2 border-t border-[var(--color-border-default)] pt-2.5">
                            ...
                            <div className="ml-auto flex flex-wrap items-center justify-end gap-1.5">
                              ...
                              >
                                {t(
                                  "cicdTemplatePage.actions.useBaseTemplate",
                                  "Use Base Template",
                                )}
                              </Button>
                            </div>
      ...
      <Modal
        ...
        title={
          editingTemplateId
            ? t("cicdTemplatePage.modal.editTitle", "Edit Template")
            : t("cicdTemplatePage.modal.createTitle", "Create Template")
        }
        footer={
          <>
            <Button
            ...
            >
              {editingTemplateId
                ? t("common.save", "Save")
                : t("cicdTemplatePage.actions.create", "Create")}
            </Button>
          </>
        }
      >
        <div className="flex flex-col gap-3">
          <Input
            ...
            placeholder={t(
              "cicdTemplatePage.form.templateIdPlaceholder",
              "e.g. web-backend-standard",
            )}
          ...
          <Input
            ...
            placeholder={t(
              "cicdTemplatePage.form.namePlaceholder",
              "e.g. Standard Web Backend",
            )}
            value={form.name}
      ...
      <ConfirmDialog
        ...
        description={t(
          "cicdTemplatePage.confirm.deleteDescription",
          "This template will no longer be shown in the list. Continue?",
        )}
        confirmLabel={t(
          "cicdTemplatePage.confirm.deleteConfirmLabel",
          "Delete Template",
        )}
