...
colors:
  ...
  # 차트 계열색 — 정체성(identity)을 나타내는 카테고리 팔레트.
  # 상태색(success/warning/error)을 계열로 재사용하지 않는다. "Limit" 을
  # warning 색으로 그리면 한도가 *경고* 로 읽힌다.
  # 색각 이상 분리도(ΔE)와 표면 대비를 검증기로 통과시킨 값이다.
  light-chart-1: "#2a78d6"
  light-chart-2: "#eb6834"
  light-chart-3: "#1baf7a"
  ...
  # 같은 세 색을 어두운 면 기준으로 다시 뽑은 단계다 (밝기 반전이 아니다).
  dark-chart-1: "#3987e5"
  dark-chart-2: "#d95926"
  dark-chart-3: "#199e70"
...
| chart-1 | `#2a78d6` | `#3987e5` | 차트 계열 1 (기준선·Request) |
| chart-2 | `#eb6834` | `#d95926` | 차트 계열 2 (Limit) |
| chart-3 | `#1baf7a` | `#199e70` | 차트 계열 3 (Current·Ready) |
...
**계열색은 `CHART_SERIES`(= `--color-chart-*`)를 쓴다. 상태색을 계열로 돌려쓰지
않는다.** 예전에는 CPU/Memory 차트의 Limit 을 `--color-warning` 으로, Current 를
`--color-success` 로 그렸다. 한도와 현재값은 **정상 설정값과 측정값**인데도 색이
"경고"·"성공"이라고 말해, 아무 문제 없는 차트가 문제 있는 것처럼 읽혔다. 상태색은
상태(Running/Failed/Pending)에만 남긴다.

`--color-chart-*` 값은 색각 이상 분리도(ΔE)와 표면 대비를 검증한 뒤 고른 것이다.
계열을 늘릴 때는 임의로 색을 만들지 말고 같은 검증을 거쳐 슬롯을 추가한다.
라이트 모드의 chart-3 은 흰 면에서 대비가 3:1 미만이라 **범례·직접 라벨이 반드시
함께 있어야 한다** (색만으로는 식별 불가).

격자는 가로선만 그린다(`vertical={false}`). 세로선까지 점선으로 깔면 격자가
데이터보다 먼저 보인다. 도넛 조각 사이는 잉크색 테두리가 아니라 **면 색 2px** 로
가른다 — 테두리가 굵으면 조각보다 테두리가 도드라진다.
