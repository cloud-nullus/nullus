...
func TestPipelineStageNames_MatchRenderedJenkinsfile(t *testing.T) {
	...
	for _, stage := range PipelineStageNames(StageOptions{}) {
		assert.Containsf(t, content, "stage('"+stage+"')",
			"선언한 단계 %q 를 Jenkinsfile 이 만들지 않는다", stage)
	}
	assert.Equal(t, len(PipelineStageNames(StageOptions{})), strings.Count(content, "stage('"),
...
func TestPipelineStageNames_MatchRenderedGitLabCI(t *testing.T) {
	_, content := renderPipelineFor(Input{Platform: port.SCMPlatformGitLab, AppName: "api", ImageTarget: jenkinsTarget()})
